...
def fire(item, url, token):
    ...
    session_id = data.get("claude_code_session_id", "unknown")
    session_url = data.get("claude_code_session_url", "unknown")
...
def main(argv=None):
    ...
    url = os.environ.get(ROUTINE_FIRE_URL_ENV_VAR)
    token = os.environ.get(ROUTINE_TOKEN_ENV_VAR)
    if not url or not token:
        print("required Routine trigger configuration is not set", file=sys.stderr)
        return 1
